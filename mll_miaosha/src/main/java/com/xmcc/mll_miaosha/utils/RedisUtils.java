package com.xmcc.mll_miaosha.utils;
...
public class RedisUtils {
    ...
    /**
     * 非原子操作，设置 key 的过期时间，与下面的 ttl（）方法同时使用
     * @param key
     * @param expire
     * @return
     *
    ...
    /**      String 数据类型方法， opsForValue表示是操作字符串类型         */
    /**
     * 设置 key 和 value
     * @param key
     * @param value
     ...
     * 根据 key 获取 value
     * @param key
     * @return 如果获取失败，返回 null ；获取成功，返回 value
     * redis：5种数据类型
     * opsForValue：表示 String 类型
     */
    ...
    /**
     * 原子操作,设置值并设置它的过期时间，等同于 下面 的 expire() 和 ttl() 方法共同使用
     * @param key
     * @param value
     * @param expire 过期时间，秒
     * @return
     ...
     * @param key
     * @param value
     * @return  如果该 key 存在就返回 false 设置不成功； key 不存在就返回 ture 设置成功
     */
     ...
     * @param key
     * @param value
     * @return  如果该 key 存在就返回之前的value；  不存在就返回 null
     */
    ...
    /**
     * 对key中filed 的 value 增加多少 如果是减少就传入负数
     * @param key
     * @param field
    ...
    public List<Object> lrange(String key, long start, long end){
        try{
            ...
            log.error("redis lrange eror,key:{},start:{},end:{},exception:{}",key,start,end,e);
            return null;
        }
    }
    /***  list  的长度，链表的总长度  **/
    ...
    /**
     * 获取key中index 位置的值，负数就反过来数，-1 为最后一个
     * @param key
     * @param index
     * @return
     */
